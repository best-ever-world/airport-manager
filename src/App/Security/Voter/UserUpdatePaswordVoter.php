...
class UserUpdatePaswordVoter extends Voter
{
    protected function supports(string $attribute, mixed $subject): bool
    {
        if (UserGroupModel::UPDATE_PASSWORD !== $attribute) {
            return false;
        }
    ...
    protected function voteOnAttribute(string $attribute, mixed $subject, TokenInterface $token): bool
    {
        ...
        if (
            in_array(UserRoleModel::ROLE_ADMIN, $currentUser->getRoles(), true)
            || in_array(UserRoleModel::ROLE_OPERATOR, $currentUser->getRoles(), true)
            && $currentUser->getUuid() === $requestedUser->getUuid()
        ) {
            return true;
        }
